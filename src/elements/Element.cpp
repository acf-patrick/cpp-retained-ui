...
namespace element
{
    ...
    unsigned int Element::getId() const { return _id; }

    style::Style Element::getStyle() const { return _style; }
    ...
    void Element::updateSpacing(const style::Spacing &spacing)
    {
        ...
        if (auto ratio = spacing.marginRatio)
            YGNodeStyleSetMarginPercent(_yogaNode, YGEdgeAll, 100 * utils::clampRatio(*ratio));

        if (auto l = spacing.marginRatioLeft)
            YGNodeStyleSetMarginPercent(_yogaNode, YGEdgeLeft, 100 * utils::clampRatio(*l));

        if (auto r = spacing.marginRatioRight)
            YGNodeStyleSetMarginPercent(_yogaNode, YGEdgeRight, 100 * utils::clampRatio(*r));

        if (auto t = spacing.marginRatioTop)
            YGNodeStyleSetMarginPercent(_yogaNode, YGEdgeTop, 100 * utils::clampRatio(*t));

        if (auto b = spacing.marginRatioBottom)
            YGNodeStyleSetMarginPercent(_yogaNode, YGEdgeBottom, 100 * utils::clampRatio(*b));

        // padding-ratio

        if (auto ratio = spacing.paddingRatio)
            YGNodeStyleSetPaddingPercent(_yogaNode, YGEdgeAll, 100 * utils::clampRatio(*ratio));

        if (auto l = spacing.paddingRatioLeft)
            YGNodeStyleSetPaddingPercent(_yogaNode, YGEdgeLeft, 100 * utils::clampRatio(*l));

        if (auto r = spacing.paddingRatioRight)
            YGNodeStyleSetPaddingPercent(_yogaNode, YGEdgeRight, 100 * utils::clampRatio(*r));

        if (auto t = spacing.paddingRatioTop)
            YGNodeStyleSetPaddingPercent(_yogaNode, YGEdgeTop, 100 * utils::clampRatio(*t));

        if (auto b = spacing.paddingRatioBottom)
            YGNodeStyleSetPaddingPercent(_yogaNode, YGEdgeBottom, 100 * utils::clampRatio(*b));
    }
    ...
    void Element::updateSize(const style::Size &size)
    {
        ...
        if (auto heightRatio = size.heightRatio)
            YGNodeStyleSetWidthPercent(_yogaNode, 100 * utils::clampRatio(*heightRatio));

        if (auto aspectRatio = size.aspectRatio)
            YGNodeStyleSetWidthPercent(_yogaNode, 100 * utils::clampRatio(*aspectRatio));
    }
}